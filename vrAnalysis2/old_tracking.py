...
sessiondb = database.get_database("vrSessions")
...
def handle_keep_planes(func):
    ...
    def wrapper(tracker_instance: "Tracker", *args, keep_planes=None, **kwargs):
        use_keep_planes = tracker_instance.get_keep_planes(keep_planes=None)
        if keep_planes is not None and not np.array_equal(keep_planes, use_keep_planes):
            msg = (
                "Keep planes is implemented but should not be used!"
                + "Since B2Session objects have their own indexing methods, we should always just let them figure out "
                + "how to handle the resulting indices!"
            )
            print(msg)
        return func(tracker_instance, *args, keep_planes=use_keep_planes, **kwargs)
...
def get_tracker(mouse_name: str, tracking_string: str = "ROICaT.tracking", try_cache: bool = True) -> "Tracker":
    ...
    cache_path = files.local_data_path() / "trackers" / f"{mouse_name}.joblib"
...
def save_tracker(tracker: "Tracker"):
    ...
    cache_path = files.local_data_path() / "trackers" / f"{tracker.mouse_name}.joblib"
...
class Tracker:
    def __init__(self, mouse_name, tracking_string="ROICaT.tracking"):
        ...
        self.tracking_string: str = tracking_string
        self.results_string: str = ".results"
        self.rundata_string: str = ".rundata"
        self.num_parts_data_path: int = len(files.local_data_path().parts)
        ...
        self.session_names: list[tuple[str, str, str]] = self.identify_tracked_sessions()
    ...
    # database utilities
    @handle_idx_ses
    def session_table(self, idx_ses: list[int] | None = None, reset_index: bool = True) -> pd.DataFrame:
        """return dataframe of requested sessions from database"""
        records = [sessiondb.getRecord(*self.sessions[ii].session_name) for ii in idx_ses]
    ...
    def process_file_name(self, filename: str, filetype: str) -> str:
        ...
        assert (
            mouseName == self.mouse_name
        ), f"mouse name found in file ({mouseName}) doesn't match mouse name of this tracker object ({self.mouse_name})!"
    ...
    def identify_tracked_sessions(self):
        ...
        for planeidx, results in enumerate(self.results):
            ...
            for stat_path, ops_path in zip(stat_paths, ops_paths):
                ...
                path_mismatch_msg = (
                    lambda s2p_output, data_path: f"{s2p_output}_path ({data_path}) does not match data_path ({files.local_data_path().parts[:self.num_parts_data_path]})!"
                )
                if not all(
                    [sp.lower() == dp.lower() for (sp, dp) in zip(stat_path.parts[: self.num_parts_data_path], files.local_data_path().parts)]
                ):
                    # Need to change effective num_parts to match (I probably moved the data from C: to D:...)
                    last_folder = files.local_data_path().parts[-1]
    ...
    def check_num_rois_per_plane(self):
        ...
        def assertion_message(planeidx: int, label, session: B2Session):
            return f"For session {session} and plane {planeidx}, # ROICaT ROIs ({len(label)}) doesn't match session ({session.get_value('roiPerPlane')[planeidx]})"
        ...
        for planeidx, results in enumerate(self.results):
            for sesidx, (labels, session) in enumerate(zip(results["clusters"]["labels_bySession"], self.sessions)):
                assert len(labels) == session.get_value("roiPerPlane")[planeidx], assertion_message(planeidx, labels, session)
                self.roi_per_plane[planeidx, sesidx] = session.get_value("roiPerPlane")[planeidx]
    ...
    def get_tracked_idx(
        ...
        idx_tracked = np.concatenate([np.stack([ucid for ucid in ucids], axis=1) for ucids in idx_to_ucid], axis=0).T
        ...
        # Return the indices and extras
        # (none here because we don't have any for the old tracker - this is for forward compatibility)
        return idx_tracked, None
    ...
    def split_by_plane(self, data: List[np.ndarray], dim=0, tracked=False, idx_ses=None, keep_planes=None):
        ...
        if tracked:
            ...
            first_last_roi = (
                np.vstack(
                    (
                        np.zeros((1, len(idx_ses))),
                        np.cumsum(self.roi_per_plane[keep_planes][:, idx_ses], axis=0),
                    )
                )
                .astype(int)
                .T
        ...
        assert all(
            [d.shape[dim] == exp_num for d, exp_num in zip(data, expected_num_rois)]
        ), f"mismatch between number of elements on dim {dim} of data and expected number of ROIs in each plane"
    ...
    def _make_lam_pix(self, idx_plane, idx_session):
        """
        ...
        roi_sparse = self._make_ROIs(idx_plane, idx_session, as_coo=False)
    ...
    def _make_ROIs(self, idx_plane, idx_session, as_coo=True):
        """
    ...
    def check_red_cell_consistency(self, idx_ses=None, keep_planes=None, use_s2p=False, s2p_cutoff=0.65):
        raise NotImplementedError("check_red_cell_consistency is not implemented yet because B2Sessions don't have the required methods")
