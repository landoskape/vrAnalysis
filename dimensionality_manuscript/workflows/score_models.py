    ...
    for model_name in tqdm(MODEL_NAMES, desc="Testing different model types"):
        ...
        for session in tqdm(sessiondb.iter_sessions(imaging=True, session_params=dict(spks_type=spks_type))):
            if clear_hyperparameters:
                model.clear_cached_hyperparameter(session)

            if clear_scores:
                model.clear_cached_score(session)

            if score_models:
                try:
                    _ = model.get_best_score(session, force_remake=force_remake, force_reoptimize=force_reoptimize)
            ...
            if check_existing_scores:
                if model.check_existing_score(session):
