"""RRRLatentsToRBFPosConfig — latent-to-latent predictability between RRR and RBFPos models.

Uses Ridge Regression (Optuna-optimized alpha) to measure how much variance
in RBFPos latents is explained by RRR latents and vice versa.
"""
...
def _gather_latents(
    ...
    rbfpos_leak_model = get_model("rbfpos_leak", registry, activity_parameters=activity_parameters_name)
    rrr_model = get_model("rrr", registry, activity_parameters=activity_parameters_name)

    hyperparameters_leak = rbfpos_leak_model.get_best_hyperparameters(session, spks_type=spks_type, method=method)[0]
    hyperparameters_rrr = rrr_model.get_best_hyperparameters(session, spks_type=spks_type, method=method)[0]

    report_leak = rbfpos_leak_model.process(session, spks_type=spks_type, hyperparameters=hyperparameters_leak)
...
class RRRLatentsToRBFPosConfig(AnalysisConfigBase):
    """Measure latent-to-latent predictability between RRR and RBFPos models.
    ...
    rrr_variance : float or str
        ...
        the same number of latents as the RBFPos leak model.
    normalize : bool
    ...
    display_name: ClassVar[str] = "rrrlatents_to_rbfpos"
    ...
    def summary(self) -> str:
        rrr_var_str = f"rrr_var={self.rrr_variance}"
        parts = [
            self.display_name,
    ...
    def process(self, session: B2Session, registry: PopulationRegistry) -> dict:
        ...
        alpha_rrr_to_pred, score_rrr_to_pred = _optimize_alpha(data["train_rrr"], data["val_rrr"], data["train_rbfpos_pred"], data["val_rbfpos_pred"])
        alpha_pred_to_rrr, score_pred_to_rrr = _optimize_alpha(data["train_rbfpos_pred"], data["val_rbfpos_pred"], data["train_rrr"], data["val_rrr"])
        alpha_rrr_to_true, score_rrr_to_true = _optimize_alpha(data["train_rrr"], data["val_rrr"], data["train_rbfpos_true"], data["val_rbfpos_true"])
        alpha_true_to_rrr, score_true_to_rrr = _optimize_alpha(data["train_rbfpos_true"], data["val_rbfpos_true"], data["train_rrr"], data["val_rrr"])
