...
from dimensionality_manuscript.env_order import ENV_SLOT_COLORS, ENV_NUM_COLORS, _session_sort_key
    ...
    def plot(self, state):
        ...
        for slot, env in enumerate(self.env_order):
            ...
            if state["show_env_labels"]:
                label = f"Env {env}" if state["label_by_env_id"] else f"Env {slot + 1}"
                ax.text(
...
def experiment_timeline(
    ...
    why the labels are slot numbers unless ``label_by_env_id`` is set.
    ...
    label_by_env_id : bool
        Label bars with the raw environment index instead of the experience slot.
