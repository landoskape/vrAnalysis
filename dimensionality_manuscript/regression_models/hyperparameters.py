...
@dataclass(frozen=True)
class PlaceFieldStructuredGainHyperparameters(PlaceFieldHyperparameters):
    """Hyperparameters for the PlaceFieldStructuredGainModel.

    The union of the place-field hyperparameters (which define the spatial maps) and the
    reduced-rank hyperparameters (which define the source->target gain map).
    ...
    smooth_width : float | None, default=1.0
        Width of the Gaussian smoothing kernel applied to place fields (in spatial units).
    rank : int, default=25
        The rank of the gain regression. Clipped at prediction time to the rank achievable
        given the number of gain units, source cells, and target cells.
    alpha : float, default=1e2
        The ridge regularization parameter for the gain regression.
    """
    ...
    @classmethod
    def get_optuna_space(cls, trial: "Trial") -> dict[str, Any]:
        """Get the Optuna search space for the PlaceFieldStructuredGainModel.
