...
def handle_inputs():
    ...
    parser.add_argument(
        "--mice", nargs="*", type=str, default=TRACKED_MICE, help="which mice to run script on"
    )
    parser.add_argument(
        "--cutoffs",
        nargs=2,
        type=float,
        default=CUTOFFS,
        help="min cutoffs for restricting cells (where relevant) to those with high reliability",
    )
    parser.add_argument(
        "--maxcutoffs",
        nargs=2,
        type=float,
        default=MAXCUTOFFS,
        help="max cutoffs for restricting cells to those with weak reliability",
    )
    parser.add_argument(
        "--keep-planes",
        nargs="*",
        type=int,
        default=KEEP_PLANES,
        help="which planes from the multiplane imaging to use",
    )
    parser.add_argument(
        "--dist-limit",
        nargs=1,
        default=DIST_LIMIT,
        type=float,
        help='determines distance limit (in pixels) for "nearest-neighbor" approach',
    )
    parser.add_argument(
        "--full-plots",
        default=False,
        action="store_true",
        help="will make all plots if used, including ones that take a long time and aren"
        "t in the main figure",
    )
    parser.add_argument(
        "--show-plots",
        default=False,
        action="store_true",
        help="show plots if used, note that there may be a lot depending on the other settings....",
    )
    parser.add_argument(
        "--save-plots",
        default=False,
        action="store_true",
        help="save plots if used, will overwrite automatically!",
    )
    parser.add_argument(
        "--save-data",
        default=False,
        action="store_true",
        help="save data produced by script if used",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        action="store_false",
        help="will process and analyze all mouse data instead of reusing saved data",
    )
...
def plot_loop_each_mouse(args):
    ...
    for mouse_name in args.mice:
        ...
        sim, corr, tracked, pwdist, nnpair, prms = roistat.make_roicat_comparison(
