...
class piezoConsistency(standardAnalysis):
    ...
    def checkFrameTiming(self, verbose=True, force=False):
        ...
        if force or not (hasattr(self, "samplePerFrame") and hasattr(self, "samplePerPlane")):
            self.samplePerFrame, self.sampleEachFrame, self.frameCounts = np.unique(
                np.diff(self.frameSamples), return_counts=True, return_inverse=True
            )
            self.samplePerPlane, self.sampleEachPlane, self.planeCounts = np.unique(
