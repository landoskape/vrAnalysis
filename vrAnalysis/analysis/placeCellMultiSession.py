...
class placeCellMultiSession(multipleAnalysis):
    ...
    def clear_pcss_data(self):
        for sesidx, pcss in enumerate(self.pcss):
            pcss.clear_data()
            self.pcss_loaded[sesidx] = False
    ...
    def make_rel_data(self, envnum, sortby=None, idx_ses=None):
        ...
        for ii, ises in enumerate(idx_ses):
            if ises != sortby:
                ...
                cmse = (
                    relmse[idx_sortby][idx_tracked_sortby[ii]],
                    relmse[ii][idx_tracked_target[ii]],
                )
                ccor = (
                    relcor[idx_sortby][idx_tracked_sortby[ii]],
                    relcor[ii][idx_tracked_target[ii]],
                )
            else:
                # compare source to test trials in source
                cmse = (
                    relmse[idx_sortby][idx_tracked_sortby[ii]],
                    relmse_test[ii][idx_tracked_target[ii]],
                )
                ccor = (
                    relcor[idx_sortby][idx_tracked_sortby[ii]],
                    relcor_test[ii][idx_tracked_target[ii]],
                )
            cred = (
                idx_red[idx_sortby][idx_tracked_sortby[ii]],
                idx_red[ii][idx_tracked_target[ii]],
            )
            mse.append(cmse)
            cor.append(ccor)
            red.append(cred)
    ...
    def make_paired_snake(
        ...
        target_mse = track_relmse[idx_target][keep_idx_reliable][
            track_pfidx
        ]  # rel-mse in target, filtered by whether reliable on sortby sessions, sorted by place field (just like snakes)
        target_cor = track_relcor[idx_target][keep_idx_reliable][
            track_pfidx
        ]  # rel-cor in target, filtered by whether reliable on sortby sessions, sorted by place field (just like snakes)
        sortby_mse = track_relmse[idx_sortby][keep_idx_reliable][
            track_pfidx
        ]  # rel-mse in sortby, filtered by whether reliable on sortby sessions, sorted by place field (just like snakes)
        sortby_cor = track_relcor[idx_sortby][keep_idx_reliable][
            track_pfidx
        ]  # rel-cor in sortby, filtered by whether reliable on sortby sessions, sorted by place field (just like snakes)
        return (
            [target_snake, sortby_snake],
            idx_red_data,
            [target_mse, sortby_mse],
            [target_cor, sortby_cor],
        )
    ...
    def measure_rel_plasticity(self, envnum, idx_ses=None, cutoffs=None, maxcutoffs=None, method="max"):
        ...
        if maxcutoffs is None:
            maxcutoffs = [np.inf, np.inf]
        store_idx_ses = copy(idx_ses)
        store_num_ses = len(store_idx_ses)
        self.load_pcss_data(idx_ses=store_idx_ses)
        target_red = []
    ...
    def plot_snake(
        ...
        self.num_ses = len(self.idx_ses)
        snake_data, sortby, idx_red = self.make_snake_data(
            envnum,
            idx_ses=idx_ses,
            sortby=sortby,
            cutoffs=cutoffs,
        ...
        if sort_by_red:
            ...
            for ii in range(self.num_ses):
                snake_data[ii] = np.concatenate(
                    (snake_data[ii][~idx_red[idx_sortby]], snake_data[ii][idx_red[idx_sortby]]),
                    axis=0,
                )
    ...
    def plot_rel_comparison(self, envnum, idx_ses=None, sortby=None, rel_method="pc", withShow=True, withSave=False):
        ...
        fig, ax = plt.subplots(
            num_rows,
            num_cols,
            figsize=(figdim * num_cols, figdim * num_rows),
            width_ratios=width_ratios,
            layout="constrained",
        )
    ...
    def hist_pfplasticity(
        ...
        fig, ax = plt.subplots(
            num_ses,
            num_ses,
            figsize=(figdim * num_ses, figdim * num_ses),
            layout="constrained",
            sharex=True,
        )
    ...
    def compare_pfplasticity(
        ...
        r2, pc, r2_stat, pc_stat, target_red, sortby_red = self.measure_pfplasticity(
            envnum, idx_ses=idx_ses, cutoffs=cutoffs, both_reliable=both_reliable
        )
        num_ses = len(idx_ses)
        ...
        ax[3].scatter(
            x_leg,
            y_leg_r2,
            s=r2_diff_norm(np.abs(y_leg_r2)) * mrksize_range[1] + mrksize_range[0],
            c=(y_leg_r2 >= 0) * 1.0,
            marker="o",
            cmap="bwr",
            vmin=0,
        ...
        ax[4].scatter(
            x_leg,
            y_leg_pc,
            s=pc_diff_norm(np.abs(y_leg_pc)) * mrksize_range[1] + mrksize_range[0],
            c=(y_leg_pc >= 0) * 1.0,
            marker="o",
            cmap="bwr",
            vmin=0,
    ...
    def plot_rel_plasticity(
        ...
        target_relmse, target_relcor, target_red, sortby_red = self.measure_rel_plasticity(
            envnum, idx_ses=idx_ses, cutoffs=cutoffs, maxcutoffs=maxcutoffs
        )
        ...
        fig, ax = plt.subplots(
            ...
            5,
            width_ratios=width_ratios,
            figsize=(sum(width_ratios), 2 * fig_dim),
            layout="constrained",
            num=1,
        )
