...
def loadBehavioralData(vrexp, distStep, speedThreshold):
    ...
    sampleDurationThresholded = (
        sampleDuration * (behaveSpeed >= speedThreshold) * withinTrialSample
    )  # keep sample duration when speed above threshold and sample within trial
    behaveSpeedThresholded = (
        behaveSpeed * (behaveSpeed >= speedThreshold) * withinTrialSample
...
def getBehaviorMaps(vrexp, distStep=(1, 5), speedThreshold=0):
    ...
    if len(distStep) == 1:
        ...
        )  # standard deviation = dsFactor if len(distStep)==2, and specified kernel width if len(distStep)==3
