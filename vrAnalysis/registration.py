...
def standard_behavior(self):
    ...
    trialEnvironmentIndex = (
        self.convertDense(trialInfo.vrEnvIdx).astype(np.int16)
        if "vrEnvIdx" in trialInfo._fieldnames
        else -1 * np.ones(self.value["numTrials"], dtype=np.int16)
    )
    ...
    rewardDelivery = (
        self.convertDense(trialInfo.rewardDeliveryFrame).astype(np.int64) - 1
    ...
    # Check shapes and sizes
    assert (
        trialEnvironmentIndex.ndim == 1 and len(trialEnvironmentIndex) == self.value["numTrials"]
...
class vrRegistration(vrExperiment):
        ...
        if len(inputs) == 1 and isinstance(inputs[0], vrExperiment):
            ...
            for key, value in userOpts.items():
                if value != self.opts[key]:
                    print(
                        f"In existing vrExperiment, opts['{key}']={self.opts[key]} but you just requested {key}={value}."
    ...
    def processBehavior(self):
        if not (self.opts["vrBehaviorVersion"] in behavior_processing.keys()):
            raise ValueError(
                f"Have not coded vrBehavior version {self.opts['vrBehaviorVersion']} yet.",
    ...
    def processImaging(self):
        ...
        self.registerValue(
            "framePerPlane",
            [F.shape[1] for F in self.loadS2P("F", concatenate=False, checkVariables=False)],
        ...
        if len(frame2time) != self.value["numFrames"]:
            if len(frame2time) - 1 == self.value["numFrames"]:
                ...
                if numMissing < 0:
                    ...
                    print(
                        f"In session {self.sessionPrint()}, frameSamples has {len(frameSamples)} elements, but {self.value['numFrames']} frames were reported in suite2p. Cannot resolve."
                    )
                ...
                if numMissing > 1:
                    print(
                        f"In session {self.sessionPrint()}, more than one frameSamples sample was missing. Consider using tiff timelineTimestamps to reproduce accurately."
                    )
                print(
                    (
                        f"In session {self.sessionPrint()}, frameSamples has {len(frameSamples)} elements, but {self.value['numFrames']} frames were saved by suite2p. "
                ...
                consistentFrames = np.all(
                    np.abs(np.log(np.diff(frame2time) / medianFramePeriod)) < np.log(1.01)
                )  # True if all frames take within 1% of median frame period
                if consistentFrames:
                    samplePeriod_f2t = np.mean(np.diff(frame2time))
                else:
                    samplePeriod_f2t = np.median(np.diff(frame2time))
                appendFrames = frame2time[-1] + samplePeriod_f2t * (
                    np.arange(numMissing) + 1
                )  # add elements to frame2time, assume sampling rate was perfect
        ...
        self.registerValue(
            "samplingDeviationMedianPercentError",
            np.exp(np.mean(np.abs(np.log(np.diff(frame2time) / np.median(np.diff(frame2time)))))),
        )
        self.registerValue(
            "samplingDeviationMaximumPercentError",
            np.exp(np.max(np.abs(np.log(np.diff(frame2time) / np.median(np.diff(frame2time)))))),
        )
