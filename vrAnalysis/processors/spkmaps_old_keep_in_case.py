...
class Maps:
    ...
    def __post_init__(self):
        ...
        if not self.by_environment:
            if self.occmap.shape != self.speedmap.shape != self.spkmap.shape[1:]:
                raise ValueError("occmap, speedmap, and spkmap must have the same shape")
        else:
            num_environments = self.occmap.shape[0]
            if len(self.speedmap) != num_environments or len(self.spkmap) != num_environments:
                raise ValueError("occmap, speedmap, and spkmap must have the same number of environments")
            for i in range(num_environments):
                if self.occmap[i].shape != self.speedmap[i].shape != self.spkmap[i].shape[1:]:
    ...
    def filter_positions(self, idx_positions: np.ndarray) -> None:
        for mapname in self.map_types():
            if self.by_environment:
                self[mapname] = [np.take(x, idx_positions, axis=-2) for x in self[mapname]]
            else:
                self[mapname] = np.take(self[mapname], idx_positions, axis=-1)

    def filter_rois(self, idx_rois: np.ndarray) -> None:
        for mapname in self.map_types():
            if self.by_environment:
                self[mapname] = [np.take(x, idx_rois, axis=-1) for x in self[mapname]]
            else:
                self[mapname] = np.take(self[mapname], idx_rois, axis=-1)
...
class Reliability:
    ...
    def __getitem__(self, key):
        """Allow numpy-style indexing of the values array.

        Parameters
        ----------
        key : int, slice, or tuple of ints/slices
            Index or indices to use for indexing the values array
...
def cached_processor(data_type: str, disable: bool = False):
    ...
    def reliability_loader(self: "SpkmapProcessor", process_method: callable, *args, **kwargs):
        ...
        if not disable and self.params.autosave:
            if result.environments == self.session.environments:
...
class SpkmapProcessor:
    ...
    def load_from_cache(self, data_type: str) -> Tuple[Union[Maps, Dict[str, np.ndarray]], bool]:
        ...
        if cache_dir.exists():
            ...
            if cached_params_path.exists():
                ...
                if self.check_params_match(cached_params):
                    return self._load_from_cache(data_type, params_hash), True
        return None, False
    ...
    def _load_from_cache(self, data_type: str, params_hash: str) -> Dict[str, np.ndarray]:
        ...
        if data_type == "raw_maps" or data_type == "processed_maps":
            ...
            return Maps(**cached_data, by_environment=False)
        elif data_type == "env_maps":
            raise NotImplementedError("Environment maps are not yet implemented")
        elif data_type == "reliability":
            raise NotImplementedError("Reliability maps are not yet implemented")
        else:
            raise ValueError(f"Unknown data type: {data_type}")
    ...
    def maps(
        ...
        if self.params.smooth_width is not None:
            maps.smooth_maps(self.dist_centers, self.params.smooth_width)
    ...
    def reliability(
        ...
        elif method == "correlation" or method == "mse":
