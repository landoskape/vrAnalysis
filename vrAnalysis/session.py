...
class vrExperiment(vrSession):
    ...
    def getFrameBehavior(self):
        ...
        assert np.all(
            [i.is_integer() for i in frameTrialIdx[count > 0]]
        ), "some neural frames were associated with multiple trials, non integers found"
        ...
        for trial in range(self.value["numTrials"]):
            ...
            if np.any(withinTrialNan):
                ...
                trialPosition[withinTrialNan] = np.interp(
                    trialTimeStamps[withinTrialNan],
                    trialTimeStamps[~withinTrialNan],
                    trialPosition[~withinTrialNan],
                )
