...
### Pointing to the right database and defining metadata
Each database is identified by a name which is associated with all the 
metadata required to load and process the database. (The above example uses
the database `vrSessions`). For any new databases, the metadata needs to be
hard-coded into the `database` [module](../vrAnalysis.database.py). Associated
with each database are several parameters. Here are two examples for two 
tables in the same Microsoft Access database file:
```python
dbdict = {
        'vrSessions': {
...
I'll add some extra explanation here, since anyone building their own database
will have to edit this dictionary for themselves.
```python
'dbPath' # the path to the database file
'dbName' # the name of the database file
'dbExt' # the extension of the database file -- this also informs the code of what program hosts the database
'tableName' # the name of the table to connect to within the database file
'uid' # the name of the field that has a unique value for each row of the database (usually an "autonumber" integer field)
'backupPath' # the path to a backup location for saving a copy of the database 
'uniqueFields' # a list of str-type pairs indicating fields for which each unique combination specifies at most a single row in the database
               # for example, there should be either 0 or 1 records in the 'vrSessions' database with:
               # - 'mouseName' == 'ATL022', 
               # - 'sessionDate' == '2023-01-01'
               # - 'sessionID' == 701
'defaultConditions' # key-value pairs to filter database lookups every time data from the database is requested
```
...
```python
df = sessiondb.getTable(suite2p=True)
```
...
```python
# by default, the sessiondb will use sessionQC=True, and useDefault=False turns that off.
sessiondb.printSessions(mouseName="ATL020", imaging=True, useDefault=False)
```

Alternatively, if you want an iterable list of `vrExperiment` objects 
referenced in the desired dataframe, you can use this:
```python
sessiondb.iterSessions(imaging=True, vrRegistration=True)
```
...
```python
sessiondb.printMiceInSessions(sessiondb.iterSessions(imaging=True, vrRegistration=True, redCellQC=False))
```
...
sessionid = '701'
sessiondb.registerSingleSession(mouseName, dateString, sessionid)
```
...
```python
sessiondb.needsRegistration(skipErrors=True)
```
...
session.
```python
sessiondb.registerSessions(maxData=30e9, skipErrors=True, **userOpts)
```
...
```python
sessiondb.printRegistrationErrors(useDefault=False)
```
...
```python
sessiondb.printRequiresSuite2P()
```
...
```python
sessiondb.updateSuite2pDateTime()
```
...
```python
validDatabase = sessiondb.checkS2P(withDatabaseUpdate=True, returnCheck=True)
```
...
```python
df = sessiondb.needsRedCellQC(mouseName='ATL028')
```

Or if instead of a dataframe you want it to print the session names, use:
```python
sessiondb.printRequiresRedCellQC()
```
...
```python
sessiondb.updateRedCellQCDateTime()
```
...
```python
ises = sessiondb.iterSessionNeedRedCellQC()
```
...
```python
success = sessiondb.setRedCellQC(<mouseName>, <dateString>, <sessionid>, state=True)
```
