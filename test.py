...
def redo_oasis(ses):
    fcorr = ses.loadfcorr(loadFromOne=False)
    ...
    # check the shape of the deconvolved array
    msg = f"In session {ses.sessionPrint()}, oasis was run and did not produce the same shaped array as suite2p spks..."
    assert ospks.shape == ses.loadS2P("spks").shape, msg

    # keep the old way around until finished...
    # ospks = []
    ...
    ses.saveone(ospks.T, "mpci.roiActivityDeconvolvedOasis")


if __name__ == "__main__":
    ises = sessiondb.iterSessions(imaging=True)  # ("ATL045", "!=")
    for ses in ises:
        if ses.loadone("mpci.roiActivityDeconvolvedOasis").dtype == np.float32:
            print(f"Skipping {ses.mouseName} {ses.dateString} {ses.sessionid} because found single ospks data!")
            continue
        print(ses.mouseName, ses.dateString, ses.sessionid)
        redo_oasis(ses)
